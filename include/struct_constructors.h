/**
 * @file struct_constructors.h
 * @author Hongzhe Yu (hyu419@gatech.edu)
 * @brief Test the struct in C++, its constructors, etc
 * @version 0.1
 * @date 2022-07-16
 * 
...
struct MPVIResults{
    ...
    void reinitialize_data(){
        _res_mean = MatrixXd::Zero(_niters, _nstates);
    ...
    void update_data(const VectorXd& new_mean, const MatrixXd& new_cov){
        if (_cur_iter < _niters){
            _res_mean.row(_cur_iter) = new_mean.transpose();
            _vec_res_covariances[_cur_iter] = new_cov;
            _cur_iter += 1;
        }
        else{
